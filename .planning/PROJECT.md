...
- [ ] HTML 코드 에디터에서 이메일 HTML 작성/붙여넣기
- [ ] 네이버 메일 렌더링 시뮬레이션
- [ ] 다음/카카오 메일 렌더링 시뮬레이션
- [ ] Gmail 렌더링 시뮬레이션
- [ ] Outlook 렌더링 시뮬레이션
- [ ] 각 클라이언트 프리뷰를 나란히 병렬 표시
- [ ] caniemail.com 등 기존 CSS 호환성 데이터 기반 시뮬레이션
- [ ] 실시간 프리뷰 반영 (HTML 수정 시 즉시 업데이트)

### Out of Scope

- 실제 스크린샷 방식 (테스트 메일 발송 후 캡처) — 인프라 복잡도, v1에서는 시뮬레이션만
- 파일 업로드 — v1은 코드 에디터 입력만 지원
- 모바일 앱 — 웹 서비스로 시작
...
---
*Last updated: 2026-04-23 after Phase 1 (Foundation) completion — ClientRuleset types, 3 client rulesets, Vitest infrastructure established*
